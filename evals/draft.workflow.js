...
- BODY, ~220-360 words, THIRD PERSON (${A.name} is the subject), in the canonical order below. ONLY "Wins" and "Insight" are required; the rest appear ONLY when genuine — omit rather than manufacture:
  ...
  "Asking — " (OPTIONAL — only a genuine, HIGH-VALUE ask, weighted by IMPORTANCE and URGENCY). He is usually NOT stuck, so the best asks are what a cohort can actually move: help TESTING or trying what he shipped, feedback on whether/HOW people would use it, a high-stakes DIRECTION call, or a real blocker. A low-stakes internal detail (which variable, which nav pattern) does NOT matter — OMIT the ask rather than surface trivia, even if it's an honest question. Never re-ask a solved problem; never end on a question just to end on one. Name a feed member only on a tight genuine match (verbatim quote + marker); else ask plainly with NO name.
