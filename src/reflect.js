...
4. BODY — ~${aimMin}-${aimMax} words (hard limits ${postspec.LENGTH.min}-${postspec.LENGTH.max}), third-person, ${name} named in the first sentence. Lead-ins appear in THIS order; "Wins" and "Insight" are REQUIRED, the rest ONLY when genuine — omit any of them rather than manufacture content (a thinner honest post beats a padded one):
   ...
   - "Asking — " (OPTIONAL — only when genuine) a real invitation for cohort input. VALID asks: (a) an open technical problem ${name} genuinely has NOT solved; (b) a request for people to try, use, or give feedback on what he shipped; (c) an honest pattern/approach question — he made a working choice and wants to hear how others handle it (e.g. "went with a single previous-view variable for the nested screens; curious what pattern people prefer"); (d) whether anyone has explored a SIMILAR idea or approach. INVALID: re-asking something he ALREADY fully solved as if it were an open blocker, inventing a struggle to justify a question, ending on a question just to end on one, or vague openness ("open to collaboration"). Don't force it — but an honest usage/pattern/idea ask is welcome even on a day with no hard blocker. Name a cohort member only on a tight, genuinely useful match (verbatim quote + marker); otherwise ask plainly with no name.
...
- THE SAME RESTRAINT GOVERNS STRUGGLES AND ASKS — report a struggle only when it was REAL friction, not a routine decision dressed as hardship. A genuine ask is welcome (an open problem he hasn't solved, a request to try/give feedback on what he built, an honest opinion-on-patterns question, or whether others have explored a similar idea) — but NEVER re-ask something he already fully solved as if it were an open blocker, never invent a struggle to justify a question, and never end on a question just to end on one. Some honest days have no Struggles, no Offering, and no Asking — that is fine.
